...
export function registerFBXLoader(THREE){		
	...
	Object.assign( FBXLoader.prototype, {
        ...
        if (this.resourceDirectory !== undefined) {

          resourceDirectory = this.resourceDirectory;

        } else if (this.path !== undefined) {

          resourceDirectory = this.path;

        } else {

					resourceDirectory = THREE.LoaderUtils.extractUrlBase(url);
			...
			if (self.crossOrigin === 'use-credentials') {
	...
	function parseImage( videoNode ) {
		...
		if ( typeof content === 'string' ) { // ASCII format
			...
			var arrayBu = wx.arrayBufferToBase64(array);
			return 'data:' + type + ';base64,' + arrayBu;			
			//ar arrayBu = new Blob( [ array ], { type: type } );
			//return window.URL.createObjectURL( arrayBu );
						// let fileManager = wx.getFileSystemManager();//获取文件管理器
						// let filePath = wx.env.USER_DATA_PATH + '/inner.png';//设置临时路径
	...
	function loadTexture( textureNode, loader, images, connections ) {
		...
		if ( textureNode.FileName.slice( -3 ).toLowerCase() === 'tga' ) {
			...
			//  let loaderTemp = new THREE.TextureLoader();
			//  fileName = "https://www.wechatvr.org/miniprogram_model/liuyi.jpg";			
			//  texture = loaderTemp.load( fileName , function (res){
			// 	console.log(res);
			//  },null, function (e){
			// 		console.log(e);
			//  });
		...
		parseProperty: function ( reader ) {
			...
			switch ( type ) {
				...
				case 'l':
					...
					var inflate = new Zlib.Zlib.Inflate( new Uint8Array( reader.getArrayBuffer( compressedLength ) ) ); // eslint-disable-line no-undef
				...
				default:
					console.log('THREE.FBXLoader: Unknown property type ' + type);
