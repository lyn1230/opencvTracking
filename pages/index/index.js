...
const templateImage = {
  ...
  imageShukoushui: [
    "https://www.wechatvr.org/imageTemplate/shukoushui_0.jpg",
    "https://www.wechatvr.org/imageTemplate/shukoushui_1.jpg",
    "https://www.wechatvr.org/imageTemplate/shukoushui_2.jpg",
    "https://www.wechatvr.org/imageTemplate/shukoushui_3.jpg",
  ],
  tempImage_id: -1,
    ...
    if (good_cnt >= 4) {
      ...
      if (!homography.empty()) {
        ...
        if (inlierSize / originLKPointSize >= Min_InlierRatio || inlierSize >= Min_InlierSize) {
          ...
          //测试光流的运动稳定性，目标左上角是x_min和y_min，宽高是width和height
          let x_min = Math.min(newVertex.data32F[0], newVertex.data32F[6]);
          let x_max = Math.max(newVertex.data32F[2], newVertex.data32F[4]);
          let y_min = Math.min(newVertex.data32F[1], newVertex.data32F[3]);
          let y_max = Math.max(newVertex.data32F[5], newVertex.data32F[7]);
